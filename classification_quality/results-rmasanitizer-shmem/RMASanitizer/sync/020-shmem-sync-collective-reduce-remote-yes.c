...
// RUN: %must-run --must:rma --must:rma-mode %must-rma-mode -np 4 %must-bin-dir/%basename_t.exe 2>&1 | %filecheck -DFILENAME=%basename_t %s
// CHECK-DAG: data race
// CHECK-DAG: [[FILENAME]]:53
// CHECK-DAG: [[FILENAME]]:58
...
/*
{
    ...
    "RACE_PAIR": ["shmem_int_sum_reduce@53","shmem_int_put@58"],
