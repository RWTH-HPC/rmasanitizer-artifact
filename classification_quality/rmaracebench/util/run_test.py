...
parser.add_argument('--tool', dest='tools', default=defaulttools, help='Select tools that should be tested (space-separated, default: all tools)', choices=['MUST-RMA', 'RMASanitizer', 'PARCOACH-dynamic', 'PARCOACH-static'], nargs='+', type=str)
...
class RunTest(ABC):
    ...
    def run_command(self, command: str, timeout = 120, envmod = {}):
        result = RunResult.SUCCESS
        self.cmd_out.write(command + '\n')
        env = os.environ.copy()
        for var in envmod:
            env[var] = envmod[var]
        p = subprocess.Popen(command.split(), stdout=subprocess.PIPE, stderr=subprocess.STDOUT, env=env)
        try:
...
class RunTestFactory:
    ...
    def createTest(self, test: Test, prefix: str, tool: str, category: str):
        if tool == 'MPIRMA':
            ...
            return RunRMASanitizerTest(test, os.path.join(prefix, tool, category))
...
class RunMUSTTest(RunTest):
    def run(self):
        ...
        command = f"/opt/must/bin/mustrun -np {self.test.nprocs} --must:distributed --must:nodl --must:output stdout --must:tsan --must:rma {binary_out}"
        self.output, self.runresult = self.run_command(command)
        self.write_stdout(self.output)
...
class RunRMASanitizerTest(RunTest):
    def run(self):
        binary_out = f'{self.source_file}.exe-must'
        command = f"/opt/rmasanitizer/bin/must-cc -g -fopenmp -lsma -lGPI2 {self.source_file} -o {binary_out}"
        self.run_command(command)

        command = f"/opt/rmasanitizer/bin/mustrun -np {self.test.nprocs} --must:output stdout --must:rma {binary_out}"
        self.output, self.runresult = self.run_command(command)
        self.write_stdout(self.output)
...
class RunParcoachStaticTest(RunTest):
    def run(self):
        ...
        self.run_command(f"mpicc -fopenmp -O0 -g -S -emit-llvm {self.source_file} -o {binary_out}.ll", envmod=clang15_env)
    ...
    if args.mode == 'plain':
        ...
        for tool in args.tools:
            ...
            for rmamodel in args.rma_models:
                ...
                for discipline in args.disciplines:
                    print(f"= {discipline} =")
                    testfiles = glob(f"/rmaracebench/{rmamodel}/{discipline}/*.c")
                    testfiles.sort()
