...
/**
 * @file VCGASPIWaitMultiple.cpp
 *       Vector clock test for notifications (2 notification sent/received)
 *
 *  @date 28.07.2023
...
// RUN: %builddir/tests/VectorClock/VCGASPIWaitMultipleLayout.xml \
// RUN: %must-bin-dir/VCGASPIWaitMultiple 2>&1 \
// RUN: | %filecheck %s

// CHECK-DAG: shutdown(0){{.*}}clk=(5, 3, 3)
// CHECK-DAG: shutdown(1){{.*}}clk=(3, 5, 3)
// CHECK-DAG: shutdown(2){{.*}}clk=(4, 4, 4)
...
int main(int argc, char* argv[])
{
    ...
    if (rank == 0 || rank == 1) {
        *(int*)src_segment_data = (rank + 1) * 10;
        gaspi_write_notify(
            loc_seg_id,
            0,
            2,
    ...
    if (rank == 2) {
        ...
        gaspi_notify_waitsome(remote_seg_id, 0, 2, &firstId, GASPI_BLOCK);
    }
