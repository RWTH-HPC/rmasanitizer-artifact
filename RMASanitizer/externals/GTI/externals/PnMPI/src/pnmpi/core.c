...
pnshmem_cell_t pnshmem_activated[NUM_MPI_CELLS];
pnshmem_functions_t pnshmem_function_ptrs;
#endif

#ifdef ENABLE_GASPI
pngaspi_cell_t pngaspi_activated[NUM_MPI_CELLS];
pngaspi_functions_t pngaspi_function_ptrs;
#endif


pnmpi_compiler_tls_keyword int pnmpi_mpi_level = 0;
int pnmpi_max_level = 0;
int pnmpi_initialization_complete = 0;
int pmpi_init_done = 0;
...
MPI_Session pnmpi_session = MPI_SESSION_NULL;
#endif
              ...
              if (modules.num > 0)
                {
                  ...
                  if (arg == NULL)
                  ...
                  else
                    {
                      ...
                      strncpy(arg->name, cmdargv[1], MAX_ARG_NAME - 1);
                      strncpy(arg->value, cmdargv[2], MAX_ARG_VALUE - 1);
  ...
  if (modules.num > 0)
    {
      ...
      #ifdef ENABLE_OPENSHMEM
      SHM_INITIALIZE_ALL_FUNCTION_STACKS(modules);
      #endif
      #ifdef ENABLE_GASPI
      GASPI_INITIALIZE_ALL_FUNCTION_STACKS(modules);
      #endif
    }
