...
/**
 *  Flag to indicate that PMPI_Init or PMPI_Init_thread has been called.
 */
extern int pmpi_init_done;

#if defined(PNMPI_ENABLE_SESSIONS)
/**
 * \brief Handle to PnMPI's own internal session.
 *
 * This session is used to perform MPI calls without having to depend and wait
 * on the application to initialize MPI.
 *
 * Set to \ref MPI_SESSION_NULL if not initialized.
 */
extern MPI_Session pnmpi_session;
#endif
...
#ifdef ENABLE_OPENSHMEM
typedef uintptr_t pnshmem_cell_t;
extern pnshmem_cell_t pnshmem_activated[NUM_MPI_CELLS];
extern pnshmem_functions_t pnshmem_function_ptrs;
#endif

#ifdef ENABLE_GASPI
typedef uintptr_t pngaspi_cell_t;
extern pngaspi_cell_t pngaspi_activated[NUM_MPI_CELLS];
extern pngaspi_functions_t pngaspi_function_ptrs;
#endif
...
#ifdef ENABLE_OPENSHMEM
#define SHM_CELL_INDEX(id)          (id / (sizeof(pnshmem_cell_t)*8))
#define SHM_CELL_BMASK(id)    (((pnshmem_cell_t)1) << (id % (sizeof(pnshmem_cell_t)*8)))
#define SHM_IS_ACTIVATED(id)  (pnshmem_activated[SHM_CELL_INDEX(id)] & SHM_CELL_BMASK(id))
#define SHM_SET_ACTIVATED(id)  pnshmem_activated[SHM_CELL_INDEX(id)]|= SHM_CELL_BMASK(id)
#define SHM_NOT_ACTIVATED(id) (!SHM_IS_ACTIVATED(id))
#endif

#ifdef ENABLE_GASPI
#define GASPI_CELL_INDEX(id)          (id / (sizeof(pngaspi_cell_t)*8))
#define GASPI_CELL_BMASK(id)    (((pngaspi_cell_t)1) << (id % (sizeof(pngaspi_cell_t)*8)))
#define GASPI_IS_ACTIVATED(id)  (pngaspi_activated[GASPI_CELL_INDEX(id)] & GASPI_CELL_BMASK(id))
#define GASPI_SET_ACTIVATED(id)  pngaspi_activated[GASPI_CELL_INDEX(id)]|= GASPI_CELL_BMASK(id)
#define GASPI_NOT_ACTIVATED(id) (!GASPI_IS_ACTIVATED(id))
#endif
...
#define SHM_RTLDNEXT_CHECK(stack) pnshmem_function_ptrs.stack[i] != __tmp_function_ptr
#define GASPI_RTLDNEXT_CHECK(stack) pngaspi_function_ptrs.stack[i] != __tmp_function_ptr
#else
#define RTLDNEXT_RETRIEVAL(r_type, routine)
#define RTLDNEXT_CHECK(stack) 1
#define SHM_RTLDNEXT_CHECK(stack) 1
#define GASPI_RTLDNEXT_CHECK(stack) 1
#endif
...
#ifdef ENABLE_OPENSHMEM
#define SHM_INITIALIZE_FUNCTION_STACK(routine, routine_id, r_type, stack, mods, \
                                  shmroutine)                               \
  {                                                                         \
    int i;                                                                  \
    DBGPRINT2("Initialize stack for %s\n", routine);                        \
    if (pnshmem_function_ptrs.stack == NULL)                                \
      {                                                                     \
        pnshmem_function_ptrs.stack =                                       \
          (r_type *)malloc(mods.num * sizeof(r_type));                      \
      }                                                                     \
    if (pnshmem_function_ptrs.stack == NULL)                                \
      {                                                                     \
    ...
    for (i = 0; i < mods.num; i++)                                          \
      {                                                                     \
        if (mods.module[i]->stack_delimiter)                                \
          continue;                                                         \
        pnshmem_function_ptrs.stack[i] =                                    \
          (r_type)find_symbol(mods.module[i], routine);                     \
        RTLDNEXT_RETRIEVAL(r_type, routine)                                 \
        WRAP_MPI_CALL_PREFIX                                                \
        if (pnshmem_function_ptrs.stack[i] != NULL &&                       \
            pnshmem_function_ptrs.stack[i] != (r_type)p##shmroutine &&      \
            SHM_RTLDNEXT_CHECK(stack))                                      \
          {                                                                 \
            SHM_SET_ACTIVATED(routine_id);                                  \
          }                                                                 \
        else                                                                \
          {                                                                 \
            pnshmem_function_ptrs.stack[i] =                                \
        ...
        WRAP_MPI_CALL_POSTFIX                                               \
        DBGPRINT2("Symbol for routine %s in module %s: value %px", routine, \
                  mods.module[i]->name, pnshmem_function_ptrs.stack[i]);    \
      }                                                                     \
  }
#endif

#ifdef ENABLE_GASPI
#define GASPI_INITIALIZE_FUNCTION_STACK(routine, routine_id, r_type, stack, mods, \
                                  gaspiroutine)                               \
  {                                                                         \
    int i;                                                                  \
    DBGPRINT2("Initialize stack for %s\n", routine);                        \
    if (pngaspi_function_ptrs.stack == NULL)                                  \
      {                                                                     \
        pngaspi_function_ptrs.stack =                                         \
          (r_type *)malloc(mods.num * sizeof(r_type));                      \
      }                                                                     \
    if (pngaspi_function_ptrs.stack == NULL)                                  \
      {                                                                     \
    ...
    for (i = 0; i < mods.num; i++)                                          \
      {                                                                     \
        if (mods.module[i]->stack_delimiter)                                \
          continue;                                                         \
        pngaspi_function_ptrs.stack[i] =                                    \
          (r_type)find_symbol(mods.module[i], routine);                     \
        RTLDNEXT_RETRIEVAL(r_type, routine)                                 \
        WRAP_MPI_CALL_PREFIX                                                \
        if (pngaspi_function_ptrs.stack[i] != NULL &&                       \
            pngaspi_function_ptrs.stack[i] != (r_type)p##gaspiroutine &&    \
            GASPI_RTLDNEXT_CHECK(stack))                                    \
          {                                                                 \
            GASPI_SET_ACTIVATED(routine_id);                                \
          }                                                                 \
        else                                                                \
          {                                                                 \
            pngaspi_function_ptrs.stack[i] =                                \
        ...
        WRAP_MPI_CALL_POSTFIX                                               \
        DBGPRINT2("Symbol for routine %s in module %s: value %px", routine, \
                  mods.module[i]->name, pngaspi_function_ptrs.stack[i]);      \
      }                                                                     \
  }
#endif
