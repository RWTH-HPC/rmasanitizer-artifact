...
{
  {{forallfn fn_name shmem_init shmem_finalize MPI_Finalize}}
...
{{fnall fn_name shmem_init shmem_finalize MPI_Finalize MPI_Pcontrol}}
...
void shmem_finalize()
{
  ...
  for (n = 1; n < size; n++)
    {
      {{forallfn fn_name shmem_finalize shmem_init MPI_Finalize}}
        if (rank > 0)
