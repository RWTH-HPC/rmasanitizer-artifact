...
inline std::string getPrefixed(std::string name) {
    // P prefix may be upper or lower case, depending on which programming framework was used.
    const char* P_prefix;
    if (name.rfind("shmem_", 0) == 0 || name.rfind("gaspi_", 0) == 0) {
        P_prefix = "p"; // GASPI and OpenSHMEM uses lower case
    } else {
        P_prefix = "P"; // Use upper case for any other, such as MPI
    }                          
    return P_prefix + name;
}
