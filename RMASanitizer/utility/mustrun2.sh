...
End-of-File
                if cmaketrue "@ENABLE_OPENSHMEM@"
                then
                cat <<End-of-File >> $printLayoutFile
                        <analysis name="ShmemTeamPredefs" group="MUST_Resource"/>
                        <analysis name="ShmemSegmentPredefs" group="MUST_Resource"/>
End-of-File
                fi
                if [ $do_rma_only -eq 0 ]
                then
...
End-of-File
                ...
                if [ $do_rma -ne 0 ]
                then
                    echo '<analysis name="MPIRMAWrapper" group="MUST_OneSidedChecks" />' >> $printLayoutFile
                    echo '<analysis name="VectorClockWrapper" group="MUST_VectorClock" />' >> $printLayoutFile
                    if cmaketrue "@ENABLE_OPENSHMEM@"
                    then
                        echo '<analysis name="SHMEMRMAWrapper" group="MUST_OneSidedChecks" />' >> $printLayoutFile
                        echo '<analysis name="SHMEMVectorClockWrapper" group="MUST_VectorClock" />' >> $printLayoutFile
                    fi
                    if cmaketrue "@ENABLE_GASPI@"
                    then
                        echo '<analysis name="GASPIRMAWrapper" group="MUST_OneSidedChecks" />' >> $printLayoutFile
                        echo '<analysis name="GASPIVectorClockWrapper" group="MUST_VectorClock" />' >> $printLayoutFile
                    fi
                    echo '<analysis name="RMASanitize" group="MUST_OneSidedChecks" />' >> $printLayoutFile
...
if cmaketrue "@ENABLE_GASPI@"
then
    gaspispecs=$(for file in                         \
      ${mustSpecDir}/gaspi_base_specification.xml    \
      ${mustSpecDir}/gaspi_specification.xml
        do
            [[ -f $file ]] && echo $file || true
        done
    )
...
if cmaketrue "@ENABLE_OPENSHMEM@"
then
    shmemspecs=$(for file in                            \
      ${mustSpecDir}/shmem_base_specification.xml       \
      ${mustSpecDir}/shmem_specification.xml    \
      ${mustSpecDir}/must_onesided_checks_api_shmem.xml
        do
            [[ -f $file ]] && echo $file || true
        done
    )
...
tsan_option_log_path="${temp_arg}/tsan.log"
