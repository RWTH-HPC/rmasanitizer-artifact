...
#define NUM_SUBMODULES 4
    ...
    myTargetChecksMod = (I_TargetChecks*)subModInstances[3];
...
TSanMessages::~TSanMessages()
{
    ...
    if (myTargetChecksMod != nullptr) {
        destroySubModuleInstance((I_Module*)myTargetChecksMod);
        myTargetChecksMod = nullptr;
    }
}
...
auto TSanMessages::tsanReport(const __tsan::ReportDescT<T>* const report) -> GTI_ANALYSIS_RETURN
{
    ...
    for (auto** it = report->mops.begin_; it != report->mops.end_; ++it) {
        ...
        if (stack->frames->next != nullptr &&
            ...
            locInfo.codeptr = (void*)(pMop->stack->frames->info.module_offset);
    ...
    for (const auto& op : rmaConflictPair) {
        ...
        myTargetChecksMod->getHistoryData(op.first, op.second, opData);
    ...
    myLogger->createMessage(MUST_WARNING_DATARACE, pId, 0, MustErrorMessage, msg.str(), refList);
    return GTI_ANALYSIS_SUCCESS;
}
