...
# This script requires the following to be set:
# - FRAMEWORK_NAME: Identifier for the specification to generate. Ex.: "OpenSHMEM", "GASPI", "MPI"
# - FRAMEWORK_SHORTNAME: Short name for the specification to generate. Also needs a "gen_api_<shortname>.h" to exist. Ex.: "shmem" for "OpenSHMEM"
# - FRAMEWORK_HEADERS: Space-seperated list of headers required for this framework, including P-funcs.
# - FRAMEWORK_FUNC_PREFIX: Prefix identifier for this framework. Ex.: "MPI" as all MPI functions start with "MPI_". Can be same as FRAMEWORK_SHORTNAME
# - FRAMEWORK_FUNC_IGNORE: Functions to disregard from the API header

tmp=tmp$$
trap "rm -f $tmp.*; exit" 0 1 2 3 9

if [ -f ./gen_api_${FRAMEWORK_SHORTNAME}.h ]
then
    src=./gen_api_${FRAMEWORK_SHORTNAME}.h
else
...
# Process ignorelist
framework_ignore_args=()
for i in ${FRAMEWORK_FUNC_IGNORE}
do
    framework_ignore_arg+=("-e" "/${i}/d")
done

grep -e " ${FRAMEWORK_FUNC_PREFIX}_.*(.*)" $src \
| sed >$tmp.tmp \
    -e '/typedef /d' \
    -e 's/( *void *)/()/' \
    -e 's/   */ /g' \
    -e 's/SHMEM_FUNCTION_ATTRIBUTES //' \
    -e 's/ SHMEM_ATTRIBUTE_DEPRECATED//' \
    -e 's/signed /signed_/g' \
    -e 's/ const \([0-9A-Za-z_*]\+\)\s*\([),]\)/_const \1\2/g' \
    -e 's/const \([0-9A-Za-z_*]\+\s\+[0-9A-Za-z_*]\+\s*[^,)]*\)/const_\1/g' \
    -e 's/long long/long_long/g' \
    -e 's/long int/long_int/g' \
    -e 's/long double/long_double/g' \
    -e 's/ /,/' \
    -e 's/(/,/' \
    -e 's/);//' \
    -e 's/, /,/g' \
    -e 's/,$//' \
    "${framework_ignore_arg[@]}"


###### XML HEADER

# Process header files
for i in ${FRAMEWORK_HEADERS}
do
    # Formatting for this line is intended! Do not change! Required for "pretty print" in XML
    framework_header_string="${framework_header_string}
		<header>${i}</header>"
done
...
<api-specification unique-name="${FRAMEWORK_NAME}">
	<api-headers>${framework_header_string}
	</api-headers>
	<functions>
End-of-File
...
# $2 is the call name
  ...
  retval = cleanupTypeQualifiers(retval)
  ...
  for (i=1; i <= num_paras; i++)
  {
    ...
    type[i] = cleanupTypeQualifiers(type[i])
 ...
 printf "\t\t\t\t<analysis order=\"pre\" name=\"CollectUnmapped:collectFunction\" group=\"MUST_CollectUnmapped\">\n"
 printf "\t\t\t\t\t<analysis-arguments>\n"
 printf "\t\t\t\t\t\t<analysis-argument type=\"operation-len\" analysis-arg-order=\"0\">\n"
 printf "\t\t\t\t\t\t\t<op-name group=\"MUST_Base\" id=\"1\">provideParallelIdStorage</op-name>\n"
 printf "\t\t\t\t\t\t</analysis-argument>\n"
 ...
 printf "\t\t\t\t</analysis>\n"
  ...
  for (i=1; i <= num_paras; i++)
  {
    ...
    if (handleName != "")
    {
    	...
    	split(handleName, temp, '"\"${FRAMEWORK_FUNC_PREFIX}_\")"'
    	opName=temp[2]
    	if (index(extra[i], "ARRAY") != 0)
...
# /*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/
# cleanupTypeQualifiers
# 
# Cleans up mangled types from initial regex parse
# /*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/
function cleanupTypeQualifiers(mangled_type)
{
	if (index(mangled_type,"const_") != 0)
    {
        sub(/const_/, "const ", mangled_type)
    }
    if (index(mangled_type,"_const") != 0)
    {
        sub(/_const/, " const", mangled_type)
    }

    if (index(mangled_type,"long_") != 0)
    {
        sub(/long_/, "long ", mangled_type)
    }

	if (index(mangled_type,"unsigned_") != 0 || index(mangled_type,"signed_") != 0)
	{
		sub(/_/, " ", mangled_type)
	}
	return mangled_type #Now unmangled
}
'
cat <<End-of-File >>$tmp.xml
	</functions>
...
gawk -f $tmp.nawk $tmp.xml > initial_${FRAMEWORK_SHORTNAME}_specification.xml
